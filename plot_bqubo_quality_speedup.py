"""
BQUBO Benchmark Visualization with Solution Quality Analysis
...
def calculate_time_to_quality(times, gaps):
    ...
    for i in range(len(configs)):
        ...
        # CQM TTQ
        if times['CQM'][i] is not None and gaps['CQM_gap'][i] is not None:
            ttq['CQM_ttq'].append(times['CQM'][i] * (1 + gaps['CQM_gap'][i] / 100))
        else:
            ttq['CQM_ttq'].append(None)
...
def plot_comprehensive_analysis(times, objectives, gaps, ttq, output_path):
    ...
    fig.suptitle('BQUBO Benchmark: Comprehensive Analysis (Time + Solution Quality)', 
    ...
    if any(times['CQM']):
        ax.plot(n_farms, times['CQM'], marker=markers['CQM'], linewidth=2.5, 
                markersize=10, color=colors['CQM'], label='CQM', alpha=0.8)
    ...
    ax.semilogy(n_farms, times['PuLP'], marker=markers['PuLP'], linewidth=2.5, 
                markersize=10, color=colors['PuLP'], label='PuLP', alpha=0.8)
    if any([t for t in times['CQM'] if t]):
    ...
    ax.plot(n_farms, objectives['PuLP'], marker=markers['PuLP'], linewidth=2.5, 
            markersize=10, color=colors['PuLP'], label='PuLP', alpha=0.8)
    if any([o for o in objectives['CQM'] if o]):
        ax.plot(n_farms, objectives['CQM'], marker=markers['CQM'], linewidth=2.5, 
                markersize=10, color=colors['CQM'], label='CQM', alpha=0.8)
    ...
    ax.plot(n_farms, gaps['PuLP_gap'], marker=markers['PuLP'], linewidth=2.5, 
            markersize=10, color=colors['PuLP'], label='PuLP', alpha=0.8)
    if any([g for g in gaps['CQM_gap'] if g is not None]):
        ax.plot(n_farms, gaps['CQM_gap'], marker=markers['CQM'], linewidth=2.5, 
                markersize=10, color=colors['CQM'], label='CQM', alpha=0.8)
    ...
    width = 0.3
    ax.bar(x - width, gaps['PuLP_gap'], width, label='PuLP', color=colors['PuLP'], alpha=0.8)
    if any([g for g in gaps['CQM_gap'] if g is not None]):
        ax.bar(x, gaps['CQM_gap'], width, label='CQM', color=colors['CQM'], alpha=0.8)
    ...
    ax.plot(n_farms, ttq['PuLP_ttq'], marker=markers['PuLP'], linewidth=2.5, 
            markersize=10, color=colors['PuLP'], label='PuLP', alpha=0.8)
    if any([t for t in ttq['CQM_ttq'] if t]):
        ax.plot(n_farms, ttq['CQM_ttq'], marker=markers['CQM'], linewidth=2.5, 
                markersize=10, color=colors['CQM'], label='CQM', alpha=0.8)
    ...
    ax.semilogy(n_farms, ttq['PuLP_ttq'], marker=markers['PuLP'], linewidth=2.5, 
                markersize=10, color=colors['PuLP'], label='PuLP', alpha=0.8)
    if any([t for t in ttq['CQM_ttq'] if t]):
        ax.semilogy(n_farms, ttq['CQM_ttq'], marker=markers['CQM'], linewidth=2.5, 
                    markersize=10, color=colors['CQM'], label='CQM', alpha=0.8)
...
def print_comprehensive_summary(times, objectives, gaps, ttq):
    ...
    print("\n" + "="*160)
    print("BQUBO COMPREHENSIVE BENCHMARK SUMMARY: TIME + SOLUTION QUALITY")
    print("="*160)
    print(f"{'N_Farms':<10} {'PuLP (s)':<12} {'CQM (s)':<12} {'DWave (s)':<12} {'PuLP Obj':<12} {'CQM Obj':<12} {'DWave Obj':<12} {'PuLP Gap%':<12} {'CQM Gap%':<12} {'DWave Gap%':<12}")
    print("-"*160)
    ...
    for i, n in enumerate(times['n_farms']):
        ...
        cqm_obj = f"{objectives['CQM'][i]:.4f}" if objectives['CQM'][i] else "N/A"
        ...
        cqm_gap = f"{gaps['CQM_gap'][i]:.2f}" if gaps['CQM_gap'][i] is not None else "N/A"
        dwave_gap = f"{gaps['DWave_gap'][i]:.2f}" if gaps['DWave_gap'][i] is not None else "N/A"
        
        print(f"{n:<10} {pulp_time:<12} {cqm_time:<12} {dwave_time:<12} {pulp_obj:<12} {cqm_obj:<12} {dwave_obj:<12} {pulp_gap:<12} {cqm_gap:<12} {dwave_gap:<12}")
    
    print("="*160)
    print("\nKEY OBSERVATIONS:")
    print("- BQUBO formulation: Binary variables only (1-acre plantation decisions)")
    print("- D-Wave shows QUALITY GAPS: Solutions are suboptimal compared to classical solvers")
    print("- Quality gaps appear even in this simpler binary formulation")
    ...
    print("- Quality-critical applications: Stick with classical solvers (PuLP)")
...
def main():
    ...
    output_path = Path(__file__).parent / "Plots" / "bqubo_comprehensive_quality_analysis.png"
