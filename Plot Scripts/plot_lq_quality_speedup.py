"""
LQ Benchmark Visualization with Solution Quality Analysis
...
def extract_times_and_objectives(data):
    ...
    for config in configs:
        ...
        # CQM times (no objectives typically stored)
        if config in data['CQM']:
            times['CQM'].append(data['CQM'][config]['result']['cqm_time'])
            objectives['CQM'].append(None)  # CQM doesn't store objective in result
        else:
...
def plot_comprehensive_analysis(times, objectives, gaps, ttq, output_path):
    ...
    fig.suptitle('LQ Benchmark: Comprehensive Analysis (Time + Solution Quality)', 
    ...
    ax.bar(x, gaps['Pyomo_gap'], width, label='Pyomo', color=colors['Pyomo'], alpha=0.8)
...
def print_comprehensive_summary(times, objectives, gaps, ttq):
    ...
    print("\n" + "="*160)
    print("LQ COMPREHENSIVE BENCHMARK SUMMARY: TIME + SOLUTION QUALITY")
    print("="*160)
    ...
    for i, n in enumerate(times['n_farms']):
        ...
        pulp_obj = f"{objectives['PuLP'][i]:.2f}" if objectives['PuLP'][i] else "N/A"
        pyomo_obj = f"{objectives['Pyomo'][i]:.2f}" if objectives['Pyomo'][i] else "N/A"
        dwave_obj = f"{objectives['DWave'][i]:.2f}" if objectives['DWave'][i] else "N/A"
    ...
    print("- D-Wave shows QUALITY GAP: Solutions are suboptimal compared to classical solvers")
    print("- This is likely due to the hybrid solver's time limit or approximation")
...
def main():
    ...
    output_path = Path(__file__).parent / "Plots" / "lq_comprehensive_quality_analysis.png"
