...
\title{Mathematical Formulations for Crop RotationYour formulation: R_c,c' = -β · 1.5 when c = c' (monoculture)
If β ∈ [-0.8, -1.5], then:

β = -0.8 → penalty = 1.2
β = -1.5 → penalty = 2.25

In log-space (if this enters exponentially):

exp(-1.2) ≈ 0.30 (70% yield loss)
exp(-2.25) ≈ 0.11 (89% yield loss)

The paper shows monoculture yields ~17-20% less, not 70-89% less. Optimization}
