#!/usr/bin/env python3
"""
Patch D-Wave CQM Benchmark Results Visualization
Creates comprehensive plots showing performance, solution quality, and constraint
validation across different problem sizes for the Patch CQM (Constrained Quadratic Model) solver.
"""
...
def load_cqm_benchmark_data(benchmark_dir):
    """Load all Patch D-Wave CQM benchmark data."""
    cqm_dir = Path(benchmark_dir) / "COMPREHENSIVE" / "Patch_DWave"
...
def plot_performance_metrics(metrics, output_path):
    ...
    fig.suptitle('Patch D-Wave CQM Performance Metrics',
...
def plot_solution_quality(metrics, output_path):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Patch D-Wave CQM Solution Quality & Validation',
                 fontsize=16, fontweight='bold', y=0.995)
    ...
    ax.set_title('Solution Objective Value (High Crop Diversity)',
                 fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.set_xticks(n_units)
    ...
    ax = axes[0, 1]
    colors_violations = [COLORS['success'] if v == 0 else COLORS['warning'] if v < 20
                         else COLORS['danger'] for v in metrics['n_violations']]
    bars = ax.bar(n_units, metrics['n_violations'], color=colors_violations,
                  alpha=0.8, width=3, edgecolor='black', linewidth=1.5)
    ax.set_xlabel('Number of Patches', fontweight='bold')
    ax.set_ylabel('Number of Violations', fontweight='bold')
    ax.set_title(
        'Constraint Violations (Multiple Crops per Patch)', fontweight='bold')
    ax.grid(True, alpha=0.3, axis='y')
    ax.set_xticks(n_units)
    ...
    for i, (x, feasible) in enumerate(zip(n_units, metrics['is_feasible'])):
        ...
        y_pos = metrics['n_violations'][i] / \
            2 if metrics['n_violations'][i] > 0 else 2
        ax.text(x, y_pos, status,
    ...
    ax.set_title('Land Utilization Rate (Over-allocation Issue)',
                 fontweight='bold')
    ax.grid(True, alpha=0.3, axis='y')
    ax.set_xticks(n_units)
    ax.legend()
    ...
    for i, (x, y) in enumerate(zip(n_units, utilization_pct)):
        ax.annotate(f'{y:.0f}%', xy=(x, y), xytext=(0, 5 if y <= 100 else -15),
                    textcoords='offset points', ha='center',
                    fontsize=9, fontweight='bold')
...
def plot_crop_diversity(metrics, output_path):
    ...
    fig.suptitle('Patch D-Wave CQM Crop Selection Patterns',
    ...
    ax.set_title('Crop Diversity (High - CQM Advantage)', fontweight='bold')
    ax.grid(True, alpha=0.3, axis='y')
    ax.set_xticks(n_units)
...
def plot_area_distribution(data, metrics, output_path):
    ...
    fig.suptitle('Patch D-Wave CQM Area Distribution Analysis',
                 fontsize=16, fontweight='bold', y=0.995)
    ...
    for idx, config in enumerate(configs):
        ...
        # Handle too many crops for pie chart
        if len(labels) > 10:
            # Keep top 9 and combine rest
            labels_display = labels[:9] + ['Other crops']
            sizes_display = sizes[:9] + [sum(sizes[9:])]
            colors_display = colors[:9] + ['#CCCCCC']
        else:
        ...
        feasible_text = 'Feasible' if d['validation'][
            'is_feasible'] else f'Infeasible ({d["validation"]["n_violations"]} viol.)'
        ax.set_title(f'{config} Patches: Crop Areas\n(Total: {d["total_area"]:.2f} ha, {feasible_text})',
                     fontweight='bold', fontsize=10)

        # Subplot 2: Patches per crop (bar chart) - top 10 crops
        ...
        ax.set_title(f'{config} Patches: Top 10 Crop Distribution\n({d["solution_summary"]["n_crops"]} crops total)',
                     fontweight='bold', fontsize=10)
    ...
    for i, config in enumerate(configs):
        ...
        color = COLORS['success'] if util <= 1.01 else COLORS['danger']
        ax_summary.text(i, allocated + 0.5, f'{util*100:.0f}%',
...
def plot_advanced_analysis(data, metrics, output_path):
    ...
    fig.suptitle('Patch D-Wave CQM Advanced Analysis',
                 fontsize=16, fontweight='bold', y=0.995)
    ...
    # Plot 2: Constraint Validation Breakdown by Type
    ax = axes[0, 1]

    # Extract constraint check data
    constraint_types = ['at_most_one_per_plot', 'x_y_linking', 'y_activation',
                        'area_bounds_min', 'food_group_constraints']
    constraint_labels = ['One/Plot', 'X-Y Link',
                         'Y Activ', 'Area Min', 'Food Grp']

    # Get data for largest config (most interesting)
    largest_config = configs[-1]
    checks = data[largest_config]['validation']['constraint_checks']

    passed = [checks[ct]['passed'] for ct in constraint_types]
    failed = [checks[ct]['failed'] for ct in constraint_types]
    ...
    ax.set_title(
        f'Constraint Breakdown ({largest_config} Patches)', fontweight='bold')
    ax.set_xticks(x)
    ...
    # Annotate totals
    for i in range(len(constraint_labels)):
        total = passed[i] + failed[i]
        if failed[i] > 0:
            ax.text(i, max(passed[i], failed[i]) + 5, f'{total}\n({failed[i]} fail)',
                    ha='center', fontweight='bold', fontsize=8, color=COLORS['danger'])

    # Plot 3: Violation Types Distribution
    ax = axes[0, 2]

    violation_counts = []
    for config in configs:
        violation_counts.append(data[config]['validation']['n_violations'])

    ax.plot(configs, violation_counts, 'o-',
            color=COLORS['danger'], linewidth=2.5, markersize=10)
    ax.set_xlabel('Number of Patches', fontweight='bold')
    ax.set_ylabel('Number of Violations', fontweight='bold')
    ax.set_title('Violations Scaling', fontweight='bold')
    ax.grid(True, alpha=0.3)

    # Annotate points
    for i, (x, y) in enumerate(zip(configs, violation_counts)):
        if y > 0:
            ax.annotate(f'{y}', xy=(x, y), xytext=(0, 5),
                        textcoords='offset points', ha='center',
    ...
    ax.set_xlabel('Number of Patches', fontweight='bold')
    ax.set_ylabel('Time (seconds)', fontweight='bold')
    ax.set_title('Time Scaling (Fast Performance)', fontweight='bold')
    ax.legend()
    ax.grid(True, alpha=0.3)

    # Plot 5: Crop Diversity vs Utilization
    ax = axes[1, 1]
    ...
    labels = [l.get_label() for l in lines]
    ax.legend(lines, labels, loc='upper left')
    ax.set_title('Diversity vs Over-allocation', fontweight='bold')
    ...
    avg_qpu = np.mean(metrics['qpu_time'])
    ...
    Avg QPU Time: {avg_qpu:.3f}s
    QPU Efficiency: ~1-2% (low overhead)
    ...
    Avg Crops: {np.mean(metrics['n_crops']):.1f}
    
    KEY ISSUES
    {'='*40}
    CQM shows constraint violations
    related to multiple crops assigned
    to the same patch.
    
    Over-allocation: {max(utilization_pct):.0f}% utilization
    at {configs[utilization_pct.index(max(utilization_pct))]} patches
    
    High crop diversity ({max(metrics['n_crops'])} crops)
    but violates one-crop-per-patch rule.
    
    Fast solve times (5-6s) with minimal
    QPU overhead make it attractive, but
...
def print_detailed_summary(data, metrics):
    ...
    print("\n" + "="*90)
    print("PATCH D-WAVE CQM BENCHMARK RESULTS SUMMARY")
    print("="*90)

    configs = sorted(data.keys())

    print(f"\n{'Config':<10} {'Vars':<8} {'Constr':<8} {'Solve(s)':<10} {'QPU(s)':<10} {'Obj':<10} {'Status':<10} {'Feasible':<10} {'Violations':<12}")
    print("-"*90)
    ...
    for config in configs:
        ...
        print(
            f"    Crops: {', '.join(d['solution_summary']['crops_selected'][:10])}")
        if len(d['solution_summary']['crops_selected']) > 10:
            print(
                f"           ... and {len(d['solution_summary']['crops_selected']) - 10} more")
        ...
        if d['validation']['n_violations'] > 0:
            print(f"    Issues:")
            for violation in d['validation']['violations'][:5]:
                print(f"      - {violation}")
...
def main():
    ...
    print("\n" + "="*90)
    print("PATCH D-WAVE CQM BENCHMARK VISUALIZATION")
    print("="*90)

    # Set up paths
    script_dir = Path(__file__).parent
    benchmark_dir = script_dir / "Benchmarks"
    output_dir = script_dir / "plots" / "patch_cqm"
    ...
    plot_performance_metrics(metrics, output_dir / "patch_cqm_performance.png")
    plot_solution_quality(metrics, output_dir / "patch_cqm_quality.png")
    plot_crop_diversity(metrics, output_dir / "patch_cqm_crop_diversity.png")
    plot_area_distribution(data, metrics, output_dir /
                           "patch_cqm_area_distribution.png")
    plot_advanced_analysis(data, metrics, output_dir /
                           "patch_cqm_advanced_analysis.png")

    print(f"\n✓ All plots saved to: {output_dir}")
    print("\nGenerated files:")
    print("  - patch_cqm_performance.png        : Time breakdown and efficiency")
    print("  - patch_cqm_quality.png            : Solution quality and validation")
    print("  - patch_cqm_crop_diversity.png     : Crop selection patterns")
    print("  - patch_cqm_area_distribution.png  : Patch and crop area distributions")
    print("  - patch_cqm_advanced_analysis.png  : Advanced metrics and correlations")
