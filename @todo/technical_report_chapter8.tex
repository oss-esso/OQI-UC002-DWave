...
\begin{itemize}
    \item Implemented a hybrid decomposition approach for MINLP farm problems
    ...
    \item Showcased reduced binary-subproblem complexity enabling better QPU fit
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Constraints}: Up to 1385 (farm scenario)
...
Critical insights from the implementation and correction process:

\begin{itemize}
    \item \textbf{Count vs. Area}: Food group constraints should count crop SELECTIONS (binary $Y$), not total area ($A$). Counting selections enforces diversity of crop types independently of allocation area.
    \item \textbf{Maximum Constraints}: Including both minimum and maximum planting area constraints prevents pathological over-allocation and improves feasibility across scenarios.
    \item \textbf{Scenario Differences}: One-hot constraints apply ONLY to patch scenarios where an entire patch must be allocated to a single crop; farm scenarios allow multiple crops per unit and therefore should not include one-hot.
\end{itemize}
