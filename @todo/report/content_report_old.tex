...
\begin{enumerate}
    \item \textbf{Study 1: Hybrid Solver Benchmarking.} We benchmark D-Wave's black-box hybrid solvers (LeapHybridCQMSampler) against classical Gurobi optimization using Formulation A across multiple scales (10 to 1,000 farms). This establishes baseline quantum performance but reveals criticalities in QPU resource utilization transparency.
    
    \item \textbf{Study 2: Pure QPU Decomposition.} We develop and evaluate eight decomposition strategies (Direct QPU, PlotBased, Multilevel, Louvain, Spectral, Coordinated, CQM-First PlotBased, HybridGrid) on Formulation A. This provides full transparency in quantum versus classical computation time, revealing that pure QPU annealing scales linearly while classical embedding overhead dominates total runtime.
    
    \item \textbf{Study 3: Quantum Advantage Demonstration.} Using Formulation B, we demonstrate practical quantum advantage where QPU achieves 3.80$\times$ higher benefit than Gurobi across 13 benchmark scenarios. This validates that problem formulation determines computational winner.
\end{enumerate}

The synthesis of these three studies provides actionable insights: quantum annealing excels when problems have quadratic structure and frustrated constraints (Study 3), classical solvers dominate on clean linear problems (Study 1), and transparent decomposition reveals that future quantum advantage requires improved hardware connectivity to eliminate embedding overhead (Study 2).
...
To establish rigorous classical baselines, we employed Gurobi 11.0.3 [add ref], widely regarded as state-of-the-art for mixed-integer programming. Gurobi implements decades of algorithmic development including:
...
\textbf{Standard BQM Form:}
\begin{equation}
\min_{\\mathbf{x} \in \{0,1\}^n} \quad E(\\mathbf{x}) = \sum_{i} h_i x_i + \sum_{i<j} J_{ij} x_i x_j
\end{equation}

where $h_i$ are linear biases (local fields) and $J_{ij}$ are quadratic couplings (interaction strengths).

\subsubsection{Complete BQM Formulation for Rotation Problem}

For the multi-period rotation problem with temporal and spatial synergies, the complete BQM formulation becomes:

\begin{align}
\min_{\\mathbf{x}} \quad E(\\mathbf{x}) &= \sum_{f,c,t} h_{(f,c,t)} \cdot x_{(f,c,t)} + \sum_{\\text{pairs}} J_{ij} x_i x_j
\end{align}

where the biases and couplings incorporate:

\textbf{Linear biases} (combining benefit and one-hot penalty):
\begin{align}
...
\begin{align}
J_{(f,c_1,t),(f,c_2,t+1)} &= -\gamma R_{c_1,c_2} A_f && \\text{(rotation synergy)} \\
J_{(f_1,c,t),(f_2,c,t)} &= -\gamma_s S_c && \\text{(spatial coupling)} \\
J_{(f,c_1,t),(f,c_2,t)} &= 2\lambda && \\text{(one-hot penalty, } c_1 \\neq c_2 \\text{)}
\end{align}

\textbf{BQM Complexity} (typical problem):
\begin{itemize}
    \item \\textbf{Variables}: $n = F \cdot C \cdot T$
    \item \\textbf{Quadratic terms}: $\\mathcal{O}(F \cdot C^2 + E \cdot C)$ where $E$ is number of farm neighbor edges
    \begin{itemize}
        \item Rotation: $F \cdot C^2 \cdot 2$ (all crop pairs, 2 time transitions)
        \item Spatial: $E \cdot C \cdot T$ (edges $\\times$ crops $\\times$ periods)
        \item One-hot: $F \cdot \binom{C}{2} \cdot T$ (crop pairs per farm-period)
    \end{itemize}
\end{itemize}

\textbf{Example scaling} (5 farms, 6 families, 3 periods):
\begin{itemize}
    \item Variables: $5 \\times 6 \\times 3 = 90$
    \item Quadratic terms: $\\approx 5 \\times 36 \\times 2 + 10 \\times 6 \\times 3 + 5 \\times 15 \\times 3 = 765$ interactions
\end{itemize}
...
\textbf{Key Metrics:}
\begin{itemize}
    \item \textbf{Embedding Time:} Classical overhead for finding the embedding (typically 0.1 to 100 seconds depending on problem connectivity)
...
This section presents the hierarchical QUBO formulation used to demonstrate quantum advantage. This formulation aggregates 27 crops into 6 families and solves the multi-period rotation problem as a QUBO on the QPU.
...
The variable represents crop families:
\[
...
To achieve quantum tractability, we aggregate 27 individual crops into 6 crop families:
...
This reduces the decision space from $|F| \times 27 \times T$ to $|F| \times 6 \times T$ variables. For a 100-farm instance with 3 periods, this yields $100 \times 6 \times 3 = 1,800$ binary decision variables.

\subsubsection{Objective Function (QUBO)}

The objective is to minimize a QUBO, which is equivalent to maximizing the negative of the objective. The signs are flipped compared to a standard MIQP:
\[
    \text{minimize} \quad H(Y) = \sum_{i} h_i Y_i + \sum_{i<j} J_{i,j} Y_i Y_j
\]

where $Y_i$ are the binary variables $Y_{f,g,t}$, and the linear ($h_i$) and quadratic ($J_{i,j}$) coefficients are derived from agronomic and economic principles.
...
\begin{itemize}
    \item $B'_g$: Aggregated family benefit (weighted combination of nutritional value, sustainability, affordability, environmental impact)
    \item $A_f$: Area of farm $f$
    \item $A_{total}$: Total area of all farms
    \item $\lambda_{div}$: Diversity bonus coefficient
    \item $\lambda_{oh}$: One-hot penalty coefficient
\end{itemize}

\paragraph{Quadratic Couplings ($J_{i,j}$)}

The quadratic couplings encode three types of interactions:

\begin{enumerate}
    \item \textbf{Temporal Synergy (Rotation):} Between variables on the same farm in consecutive years.
    \[ J_{(f,g_1,t-1), (f,g_2,t)} = -\gamma_{rot} \frac{A_f}{A_{total}} \cdot R'_{g_1,g_2} \]
    
    where $R'$ is a $6 \times 6$ family synergy matrix encoding:
    \begin{itemize}
        \item \textbf{Monoculture penalty}: $R'_{g,g} < 0$ (same crop depletes nutrients)
        \item \textbf{Disease carryover}: Negative values for susceptible pairs
        \item \textbf{Beneficial rotations}: Positive values (e.g., legumes $\to$ grains)
    \end{itemize}
    ...
    This models pollination, pest spread, and shared resources between neighboring farms.
    ...
    This soft constraint penalizes planting multiple families on the same farm-period.
\end{enumerate}

\subsubsection{Constraints}

Unlike a traditional MIQP, the QUBO formulation encodes constraints as penalties in the objective. However, we do impose hard bounds:

\begin{enumerate}
    \item \textbf{Maximum Families (Hard Bound):} To maintain feasibility:
    \[ \forall f \in F, \forall t \in T: \quad \sum_{g \in G} Y_{f,g,t} \le 2 \]
    
    \item \textbf{Minimum Families (Hard Bound):} Ensures at least one family planted:
    \[ \forall f \in F, \forall t \in T: \quad \sum_{g \in G} Y_{f,g,t} \ge 1 \]
    
    \item \textbf{Rotation Constraint:} Prevents planting the same family in consecutive years:
    \[ \forall f \in F, \forall g \in G, \forall t \in \{1, \dots, T-1\}: \quad Y_{f,g,t} + Y_{f,g,t+1} \le 1 \]
\end{enumerate}

\paragraph{Key Innovation: Frustrated Synergies}

The rotation matrix $R'$ is constructed with 70 to 88\% antagonistic interactions (negative entries) to create a frustrated system:
\[
R'_{g_1,g_2} = \begin{cases}
-\beta \cdot 1.5 & \text{if } g_1 = g_2 \text{ (monoculture)} \\
\text{Unif}(-1.2\beta, -0.3\beta) & \text{with prob. } 0.70 \text{ to } 0.88 \text{ (frustrated)} \\
\text{Unif}(0.02, 0.20) & \text{otherwise (beneficial)}
\end{cases}
\]

This frustration creates a rugged energy landscape that:
\begin{itemize}
    \item Defeats classical branch-and-bound pruning
    \item Creates massive integrality gaps (greater than 700\%)
    \item Enables quantum advantage through tunneling
\end{itemize}
...
We conducted extensive benchmarking of D-Wave's hybrid solvers (LeapHybridCQMSampler, LeapHybridBQMSampler) against classical Gurobi optimization across two test configurations:
...
\textbf{Implication:} Quantum annealing provides computational advantage specifically in the QUBO formulation space. This is not a universal advantage (classical CQM solvers dominate), but a \textit{formulation-dependent} advantage where problem encoding determines which computational paradigm succeeds. The BQM hybrid solver's ability to handle 14+ million interactions demonstrates quantum advantage at scale.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/01_top_crop_distribution.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/02_benefit_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../../professional_plots/violation_impact_assessment.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../plots/qpu_benchmark_comprehensive.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../plots/qpu_solution_composition_pies.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../plots/comprehensive_scaling.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../plots/variable_count_scaling_analysis.png}
...
D-Wave's LeapHybridCQMSampler achieves impressive performance: 0\% optimality gap with constant 5 to 12 second solve times across all scales (10 to 1,000 farms). However, post-hoc analysis revealed that pure QPU annealing constitutes less than 5\% of total wall-clock time. The remaining 95\% is classical preprocessing (problem decomposition, embedding search) and postprocessing (solution refinement). This finding demonstrates that \textit{claimed quantum performance is actually dominated by classical algorithms}.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../plots/plot_time_vs_vars.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../plots/qpu_solution_composition_pies.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../plots/plot_gap_speedup_vs_vars.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../plots/plot_solution_quality_vs_vars.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../plots/qpu_solution_composition_histograms.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/qpu_benchmark_comprehensive.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/plot_time_vs_vars.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/plot_gap_speedup_vs_vars.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/plot_time_comparison.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/plot_solution_quality_vs_vars.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/qpu_solution_composition_pies.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/qpu_solution_composition_histograms.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/01_top_crop_distribution.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/qpu_solution_detail_100farms.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/qpu_solution_detail_500farms.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/qpu_solution_detail_1000farms.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/qpu_solution_food_groups.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/02_benefit_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/03_ranking_variability.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/04_sensitivity_w_nutr_val.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/04_sensitivity_w_nutr_den.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/04_sensitivity_w_env_imp.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/04_sensitivity_w_afford.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/04_sensitivity_w_sustain.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/05_spinach_analysis.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../plots/06_parallel_coordinates.png}
