...
\begin{quote}
    \textit{Given a collection of land parcels, which crop should be assigned to each parcel to maximize overall benefit while satisfying dietary diversity and area constraints?}
\end{quote} 
...
Note the critical difference: in BP, $Y_{p,c}$ represents a \emph{complete assignment} of plot $p$ to crop $c$, not merely an indicator. Each plot has fixed area $L_p$ (typically equal across plots in a uniform grid).
...
% \begin{equation}
% Y_{p,c,t} + Y_{f,c,t+1} \leq 1 \quad \forall f \in \mathcal{F}, c \in \mathcal{C}, t \in \{1,2\}
% \end{equation}
...
\begin{itemize}
    ...
    \item \textbf{Spatial interaction treatment:} Traditional approaches impose hard constraints forbidding same-family crops on adjacent plots \cite{ballot2023first}, while ecological studies question whether spatial concentration actually worsens pest impacts or not \cite{karp2018crop, root1973organization}. Our soft penalty approach bridges these conflicting findings.
    
    \item \textbf{Constraint structure:} Some models enforce rotation through explicit temporal constraints ($Y_{p,c,t} + Y_{f,c,t+1} \leq 1$), others through objective function penalties, and still others through eligibility sets that pre-filter valid crop sequences \cite{haneveld2005crop}.
\end{itemize}
...
\[
    ...
    1 & \text{if crop } c \in C \text{ is planted on farm } p \in \mathcal{P} \text{ in period } t \in T \\
    0 & \text{otherwise}
    \end{cases}
\]
...
\begin{equation}
\text{Temporal} = \gamma_{\text{rot}} \sum_{p \in \mathcal{P}} \sum_{t=2}^{T} \sum_{c_1 \in C} \sum_{c_2 \in C} \frac{L_p}{A_{\text{total}}} \cdot R_{c_1,c_2} \cdot Y_{p,c_1,t-1} \cdot Y_{p,c_2,t}
\end{equation}
...
\begin{equation}
\text{Spatial} = \gamma_{\text{spat}} \sum_{(p_1, p_2) \in \mathcal{E}} \sum_{t \in T} \sum_{c_1 \in C} \sum_{c_2 \in C} \frac{S_{c_1,c_2}}{A_{\text{total}}} \cdot Y_{p_1,c_1,t} \cdot Y_{p_2,c_2,t}
\end{equation}
...
\begin{equation}
\text{Diversity} = \lambda_{div} \sum_{p \in \mathcal{P}} \sum_{c \in C} \mathbb{I}\left(\sum_{t \in T} Y_{p,c,t} > 0\right)
\end{equation}
...
\begin{equation}
J_{(p,c_1),(p,c_2)} = 2\lambda_{\text{oh}} \quad \forall c_1 \neq c_2
\end{equation}
...
\begin{align}
J_{(p,c_1,t),(p,c_2,t+1)} &= -\gamma_{\text{rot}} \cdot \frac{R_{c_1,c_2} \cdot L_p}{A_{\text{total}}} && \text{(from Temporal)} \\
J_{(p_1,c_1,t),(p_2,c_2,t)} &= -\gamma_{\text{spat}} \cdot \frac{S_{c_1,c_2}}{A_{\text{total}}} && \text{(from Spatial, } (p_1,p_2) \in \mathcal{E} \text{)} \\
J_{(p,c_1,t),(p,c_2,t)} &= 2\lambda_{\text{oh}} && \text{(from Penalty, } c_1 \neq c_2 \text{)}
\end{align}
...
\begin{itemize}
    ...
    \item \textbf{Binary Plot (BP):} Discretizes each farm into fixed-area plots, each assigned to exactly one crop via binary variables $Y_{p,c} \in \{0,1\}$. This formulation has $27 \times |\mathcal{P}|$ binary variables where $|\mathcal{P}|$ is the number of plots.
\end{itemize}
...
\midrule
3 & 189 & MILP (Farm w/Afc) & without proof & 0.01s & optimal \\
3 & 189 & MILP (Farm w/Afc) & with proof & 0.00s & optimal \\
3 & 108 & BP (Binary Plot) & without proof & 0.01s & infeasible \\
3 & 108 & BP (Binary Plot) & with proof & 0.01s & infeasible \\
\midrule
11 & 621 & MILP (Farm w/Afc) & without proof & 0.01s & optimal \\
11 & 621 & MILP (Farm w/Afc) & with proof & 0.09s & optimal \\
11 & 324 & BP (Binary Plot) & without proof & 0.01s & optimal \\
11 & 324 & BP (Binary Plot) & with proof & 0.00s & optimal \\
\midrule
36 & 1,971 & MILP (Farm w/Afc) & without proof & 0.02s & optimal \\
36 & 1,971 & MILP (Farm w/Afc) & with proof & 0.00s & optimal \\
36 & 999 & BP (Binary Plot) & without proof & 0.01s & optimal \\
36 & 999 & BP (Binary Plot) & with proof & 0.02s & optimal \\
\midrule
116 & 6,291 & MILP (Farm w/Afc) & without proof & 0.01s & optimal \\
116 & 6,291 & MILP (Farm w/Afc) & with proof & 0.01s & optimal \\
116 & 3,159 & BP (Binary Plot) & without proof & 0.02s & optimal \\
116 & 3,159 & BP (Binary Plot) & with proof & 0.02s & optimal \\
\midrule
369 & 19,953 & MILP (Farm w/Afc) & without proof & 0.03s & optimal \\
369 & 19,953 & MILP (Farm w/Afc) & with proof & 0.03s & optimal \\
369 & 9,990 & BP (Binary Plot) & without proof & 0.04s & optimal \\
369 & 9,990 & BP (Binary Plot) & with proof & 0.05s & optimal \\
\midrule
1,170 & 63,207 & MILP (Farm w/Afc) & without proof & 0.10s & optimal \\
1,170 & 63,207 & MILP (Farm w/Afc) & with proof & 0.10s & optimal \\
1,170 & 31,617 & BP (Binary Plot) & without proof & 0.16s & optimal \\
1,170 & 31,617 & BP (Binary Plot) & with proof & 0.16s & optimal \\
\midrule
3,703 & 199,989 & MILP (Farm w/Afc) & without proof & 0.34s & optimal \\
3,703 & 199,989 & MILP (Farm w/Afc) & with proof & 0.33s & optimal \\
3,703 & 100,008 & BP (Binary Plot) & without proof & 0.40s & optimal \\
3,703 & 100,008 & BP (Binary Plot) & with proof & 0.33s & optimal \\
\midrule
11,711 & 632,421 & MILP (Farm w/Afc) & without proof & 1.54s & optimal \\
11,711 & 632,421 & MILP (Farm w/Afc) & with proof & 1.49s & optimal \\
11,711 & 316,224 & BP (Binary Plot) & without proof & 2.00s & optimal \\
11,711 & 316,224 & BP (Binary Plot) & with proof & 1.79s & optimal \\
\midrule
37,036 & 1,999,971 & MILP (Farm w/Afc) & without proof & 3.53s & optimal \\
37,036 & 1,999,971 & MILP (Farm w/Afc) & with proof & 3.42s & optimal \\
37,036 & 999,999 & BP (Binary Plot) & without proof & 13.55s & optimal \\
37,036 & 999,999 & BP (Binary Plot) & with proof & 7.15s & optimal \\
\midrule
74,073 & 3,999,969 & MILP (Farm w/Afc) & without proof & 8.36s & optimal \\
74,073 & 3,999,969 & MILP (Farm w/Afc) & with proof & 8.65s & optimal \\
74,073 & 1,999,998 & BP (Binary Plot) & without proof & 376.45s & optimal \\
74,073 & 1,999,998 & BP (Binary Plot) & with proof & 264.66s & optimal \\
\midrule
148,147 & 7,999,965 & MILP (Farm w/Afc) & without proof & 14.17s & optimal \\
148,147 & 7,999,965 & MILP (Farm w/Afc) & with proof & 14.57s & optimal \\
...
\caption{Study~1.A hybrid benchmark (10--1{,}000 plots). Left: runtime scaling for Gurobi CQM, D-Wave CQM hybrid, Gurobi BQM/QUBO, and D-Wave BQM hybrid. Right: absolute objective gap to Gurobi with healed-gap overlays and constraint-violation bars. Healed gaps are computed after repairing one-crop-per-patch violations.}
...
\caption{QPU decomposition benchmark for large-scale problems (200--1,000 farms). The updated interpretation emphasizes full-span methods and separates raw objective quality from feasibility-aware quality using healed diagnostics. Coordinated attains lower raw gap but accumulates one-hot violations; Multilevel(10) and CQM-First remain more stable in feasibility terms.}
...
\caption{Comprehensive QPU decomposition benchmark spanning all problem scales (10--1,000 farms). The updated comparison includes feasibility-aware interpretation (healed gaps) and Gurobi-decomposed references to separate decomposition-induced effects from QPU sampling effects.}
...
\begin{tabular}{rccccc}
\toprule
\textbf{Farms} & \textbf{Partitions} & \textbf{QPU (s)} & \textbf{Embedding (s)} & \textbf{Total (s)} & \textbf{QPU\%} \\
\midrule
...
QPU time remains under 30 seconds even for 1,000-farm problems. The bottleneck is \textit{classical embedding}, which in the updated benchmark accounts for roughly 76--86\% of total runtime at large scales. This confirms that QPU sampling scales favorably while embedding remains the dominant engineering bottleneck.

\item \textbf{Coordinated gives lower raw gap; Multilevel(10) and CQM-First remain more feasibility-robust.}

At 1,000 farms (Table~\ref{tab:quality_1000farms}), Coordinated achieves a lower raw gap but accumulates one-hot violations. Multilevel(10) and CQM-First remain zero-violation at this scale in QPU runs and are therefore more stable in healed comparisons.

\begin{table}[H]
\centering
\caption{Solution quality at 1,000-farm scale (updated). ``(h)'' rows indicate healed objective values.}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Solution quality scaling: QPU Hierarchical (solid lines) versus Gurobi 300\,s timeout (dotted lines) objective values, separated by formulation type (6-Food and 27-Food Aggregated). \textit{Left:} Scaling by number of farms; QPU benefit diverges increasingly from Gurobi as farm count grows, while Gurobi objective grows slowly due to timeout-limited exploration. \textit{Right:} The same data plotted against total decision variables; 6-Food and 27-Food (Agg.) traces overlap in variable range because the two formulations reach similar variable counts via different combinations of farm and crop-per-farm. The exception at 90 variables (\texttt{rotation\_micro\_25}) is the sole scenario where Gurobi finds the global optimum ($0.79\times$ ratio).}
...
In the latest timeout verification sweep (20 scenarios, 200~s cap), Gurobi times out on 11/20 scenarios overall. Timeout behavior is highly formulation-dependent: 6-Food times out on 10/10 scenarios, whereas 27-Food aggregated solves to optimality on 9/10 scenarios, with only the 1,000-farm case timing out.
...
Since 6-Food has fewer crops per farm (18 vars/farm) than 27-Food (Agg.) (81 vars/farm), it requires \textbf{4.5$\times$ more farms} to reach the same variable count, resulting in 4.5$\times$ more clusters and thus $\sim$4$\times$ higher solve time. The pure QPU access time per cluster is nearly identical ($\sim$0.12s); the difference comes entirely from classical embedding overhead that scales with cluster count. See Section~\ref{sub:clusterbased} for the mathematical details of the cluster-based decomposition.
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{QPU solve time breakdown on log scale. \textit{Solid lines:} QPU total wall time (embedding + classical overhead + quantum access). \textit{Dashed lines:} pure QPU access time only. \textit{Dotted lines:} Gurobi solve time (near-flat at 300\,s timeout for 11 of 13 scenarios). \textit{Left:} Scaling by number of farms. \textit{Right:} Scaling by number of decision variables. The gap between solid and dashed curves represents classical embedding and coordination overhead, which accounts for 97--99\% of total QPU wall time across all scenarios. The 6-Food formulation (blue) incurs higher wall time per variable than 27-Food (Agg.) (orange) because it requires $\sim$4.5$\times$ more farm clusters at the same variable count (Section~\ref{sec:formulation_timing}). Pure QPU access time (dashed) remains below 1\,s per call regardless of scale.}
...
The timing analysis (Table~\ref{tab:timing_comparison}) confirms that the embedding bottleneck pattern observed in Study 2.A persists in Study 2.B: QPU time totals only 10.88~seconds across all 13~scenarios (1.1\% of wall time), scaling linearly at $T = 0.78 \cdot N_{\text{vars}} + 51$\,ms. Classical embedding and coordination consume the remaining 99\% of runtime. The formulation-specific analysis (Figure~\ref{fig:split_analysis}) further reveals that this overhead scales with \textit{farm count} rather than variable count.
...
If embedding overhead were eliminated entirely, the 1,000-farm problem would complete in the tens-of-seconds regime rather than the current decomposition wall time. This acceleration would place QPU workflows much closer to classical runtimes on Variant~A while substantially improving practical competitiveness on difficult Variant~B timeout regimes.
...
Our decomposition approach is inherently parallelizable: independent farm clusters can be solved simultaneously on separate QPUs. For the 1,000-farm Multilevel(10) benchmark (102~partitions), a hypothetical 10-QPU array would reduce QPU time from 21.78\,s to $\sim$2.2\,s. The embedding bottleneck would also parallelize: each QPU independently embeds its assigned partitions, reducing embedding wall-clock time by a factor of 10.
...
\item \textbf{Violations}: Can occur when farms in the same partitions compete for crops
\end{itemize}
...
\For{each farm $f \in \mathcal{F}$}
    \State Build farm BQM with objective $\max \sum_{c \in \text{selected\_crops}} b_c \cdot L_p \cdot Y_{f,c}$
...
For each farm $f \in \mathcal{F}$, we construct a farm-specific optimization problem over variables $\{Y_{p,c,t}\}_{c \in \mathcal{C}, t \in \mathcal{T}}$ where $\mathcal{C}$ represents the 6 crop families and $\mathcal{T} = \{1,2,3\}$ represents the three time periods. The farm subproblem objective incorporates three components:

\begin{align}
Z_f = & \sum_{c \in \mathcal{C}} \sum_{t \in \mathcal{T}} \frac{B_c \cdot A_f}{A_{\text{total}}} \cdot Y_{p,c,t} \label{eq:farm_benefit} \\
& + \sum_{t=2}^{T} \sum_{c_1, c_2 \in \mathcal{C}} \frac{\gamma_{\text{rot}} \cdot R_{c_1, c_2} \cdot A_f}{A_{\text{total}}} \cdot Y_{p,c_1,t-1} \cdot Y_{p,c_2,t} \label{eq:farm_rotation} \\
...
& + \sum_{(p_1,p_2) \in \mathcal{E}_i} \sum_{c_1,c_2 \in \mathcal{C}} \frac{\gamma_{\text{spat}} \cdot S_{c_1,c_2}}{A_{\text{total}}} \cdot Y_{p_1,c_1,t} \cdot Y_{p_2,c_2,t} \label{eq:intra_cluster_spatial}
\end{align}
...
\begin{equation}
\text{Rotation term:} \quad \sum_{f \in \mathcal{F}_i} \sum_{c_1,c_2 \in \mathcal{C}} \frac{\gamma_{\text{rot}} \cdot R_{c_1,c_2} \cdot A_f}{A_{\text{total}}} \cdot Y_{p,c_1,t-1}^* \cdot Y_{p,c_2,t}
\end{equation}

where $Y_{p,c_1,t-1}^*$ are fixed assignments from the previous temporal slice.
...
\toprule
\textbf{Formulation} & \textbf{Vars} & \textbf{Farms} & \textbf{Clusters} & \textbf{QPU Time (s)} & \textbf{Gurobi Time (s)} & \textbf{Gurobi MILP Gap} \\
\midrule
...
\textbf{Scenario} & \textbf{Farms} & \textbf{Vars} & \textbf{Obj.} & \textbf{Time (s)} & \textbf{MILP Gap} & \textbf{Status} & \textbf{Obj.} & \textbf{Wall (s)} & \textbf{QPU (s)} & \textbf{Ratio} & \textbf{Viol.} \\
\midrule
...
rotation\_15farms\_6foods & 15 & 270 & 9.68 & 26.3 & 361.4\% & feasible & 26.22 & 17.3 & 0.24 & 2.71$\times$ & 10 \\
rotation\_medium\_100 & 20 & 360 & 12.78 & 100.2 & 260.2\% & timeout & 39.24 & 25.1 & 0.36 & 3.07$\times$ & 13 \\
rotation\_25farms\_6foods & 25 & 450 & 13.45 & 100.2 & 573.2\% & timeout & 52.67 & 26.2 & 0.37 & 3.92$\times$ & 17 \\
rotation\_large\_200 & 40 & 720 & 21.57 & 100.4 & 215.3\% & timeout & 94.64 & 75.7 & 0.75 & 4.39$\times$ & 32 \\
rotation\_50farms\_6foods & 50 & 900 & 26.92 & 100.7 & 215.8\% & timeout & 109.67 & 52.3 & 0.75 & 4.07$\times$ & 34 \\
rotation\_75farms\_6foods & 75 & 1,350 & 40.37 & 100.8 & 215.9\% & timeout & 161.44 & 78.7 & 1.09 & 4.00$\times$ & 54 \\
rotation\_100farms\_6foods & 100 & 1,800 & 53.77 & 100.7 & 176,411\% & timeout & 229.14 & 106.8 & 1.49 & 4.26$\times$ & 79 \\
\midrule
\multicolumn{12}{l}{\textit{27-Food (Aggregated) Formulation}} \\
\midrule
rotation\_25farms\_27foods & 25 & 2,025 & 11.68 & 103.8 & 258.0\% & timeout & 57.60 & 29.9 & 0.37 & 4.93$\times$ & 16 \\
rotation\_50farms\_27foods & 50 & 4,050 & 23.36 & 112.0 & 257.8\% & timeout & 102.61 & 59.3 & 0.75 & 4.39$\times$ & 32 \\
rotation\_100farms\_27foods & 100 & 8,100 & 46.68 & 134.0 & 379.5\% & timeout & 235.11 & 177.6 & 1.49 & 5.04$\times$ & 74 \\
rotation\_200farms\_27foods & 200 & 16,200 & 93.52 & 142.2 & 296.6\% & timeout & 500.59 & 323.2 & 2.87 & 5.35$\times$ & 157 \\
\midrule
...
\item Gurobi times out on 11/13 scenarios. The two non-timeout cases are the smallest (optimal solution found) and rotation\_15farms\_6foods (feasible but with 361\% MILP gap).
\item The QPU benefit ratio increases monotonically with problem size: from 0.79$\times$ (5 farms) to 5.35$\times$ (200 farms).
\item QPU access time ranges from 0.12\,s (5 farms) to 2.87\,s (200 farms), constituting 0.9--1.5\% of total wall time.
\item One-hot violations scale approximately linearly with farm count (0.2--0.8 violations per farm).
\item 6-Food formulation shows higher benefit/farm (due to denser constraint satisfaction) but larger MILP gaps at scale.
\end{enumerate}
...
\toprule
\textbf{Scenario} & \textbf{Farms} & \textbf{Foods} & \textbf{Vars} & \textbf{Objective} & \textbf{Time (s)} & \textbf{MILP Gap} \\
\midrule
...
rotation\_15farms\_6foods & 15 & 6 & 270 & 9.460 & 200.28 & 80.335\% \\
rotation\_medium\_100 & 20 & 6 & 360 & 12.506 & 200.32 & 90.040\% \\
rotation\_25farms\_6foods & 25 & 6 & 450 & 13.450 & 200.24 & 89.573\% \\
rotation\_large\_200 & 40 & 6 & 720 & 21.566 & 200.40 & 97.036\% \\
rotation\_50farms\_6foods & 50 & 6 & 900 & 26.892 & 200.94 & 98.470\% \\
rotation\_75farms\_6foods & 75 & 6 & 1,350 & 40.365 & 200.69 & 98.262\% \\
rotation\_100farms\_6foods & 100 & 6 & 1,800 & 53.840 & 200.73 & 99.157\% \\
rotation\_150farms\_6foods & 150 & 6 & 2,700 & 80.542 & 203.68 & 99.372\% \\
\midrule
\multicolumn{7}{l}{\textit{27-Food (Aggregated) Formulation}} \\
\midrule
rotation\_25farms\_27foods & 25 & 27 & 2,025 & 10.985 & 2.33 & 0.0\% \\
rotation\_50farms\_27foods & 50 & 27 & 4,050 & 21.941 & 3.84 & 0.0\% \\
rotation\_75farms\_27foods & 75 & 27 & 6,075 & 33.390 & 4.87 & 0.0\% \\
rotation\_100farms\_27foods & 100 & 27 & 8,100 & 44.525 & 9.49 & 0.0\% \\
rotation\_150farms\_27foods & 150 & 27 & 12,150 & 66.496 & 17.46 & 0.0\% \\
rotation\_200farms\_27foods & 200 & 27 & 16,200 & 88.939 & 34.43 & 0.0\% \\
rotation\_250farms\_27foods & 250 & 27 & 20,250 & 111.828 & 43.15 & 0.0\% \\
rotation\_350farms\_27foods & 350 & 27 & 28,350 & 154.253 & 58.30 & 0.0\% \\
rotation\_500farms\_27foods & 500 & 27 & 40,500 & 219.701 & 183.01 & 0.0\% \\
rotation\_1000farms\_27foods & 1,000 & 27 & 81,000 & 437.971 & 202.27 & 2,518.892\% \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
\item In contrast, 27-Food aggregated is solved to optimality in 9/10 scenarios; only the 1,000-farm case times out with very large gap.
...
rotation\_15farms\_6foods & 270 & 17.27 & 0.244 & 0.150 & 1.4\% \\
rotation\_medium\_100 & 360 & 25.07 & 0.362 & 0.220 & 1.4\% \\
rotation\_25farms\_6foods & 450 & 26.16 & 0.371 & 0.229 & 1.4\% \\
rotation\_large\_200 & 720 & 75.69 & 0.747 & 0.463 & 1.0\% \\
rotation\_50farms\_6foods & 900 & 52.28 & 0.747 & 0.463 & 1.4\% \\
rotation\_75farms\_6foods & 1,350 & 78.68 & 1.093 & 0.667 & 1.4\% \\
rotation\_100farms\_6foods & 1,800 & 106.83 & 1.487 & 0.920 & 1.4\% \\
rotation\_25farms\_27foods & 2,025 & 29.88 & 0.371 & 0.229 & 1.2\% \\
rotation\_50farms\_27foods & 4,050 & 59.27 & 0.747 & 0.463 & 1.3\% \\
rotation\_100farms\_27foods & 8,100 & 177.56 & 1.487 & 0.920 & 0.8\% \\
rotation\_200farms\_27foods & 16,200 & 323.20 & 2.871 & 1.784 & 0.9\% \\
\midrule
...
\begin{table}[H]
\centering
\caption{Study 2.B: Constraint violation detail per scenario. ``Slots'' = farms $\times$ periods (total allocation decisions); ``Violations'' = one-hot constraint failures (farm-period with no crop assigned); ``Rate'' = Violations / Slots. All violations are one-hot type; no food-group violations were observed.}
\label{tab:study2b_violations_detail}
\small
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{l r r r r r r}
\toprule
\textbf{Scenario} & \textbf{Farms} & \textbf{Periods} & \textbf{Slots} & \textbf{Violations} & \textbf{Rate} & \textbf{Benefit/Slot} \\
\midrule
rotation\_micro\_25 & 5 & 3 & 15 & 1 & 6.7\% & 0.324 \\
rotation\_small\_50 & 10 & 3 & 30 & 7 & 23.3\% & 0.726 \\
rotation\_15farms\_6foods & 15 & 3 & 45 & 10 & 22.2\% & 0.583 \\
rotation\_medium\_100 & 20 & 3 & 60 & 13 & 21.7\% & 0.654 \\
rotation\_25farms\_6foods & 25 & 3 & 75 & 17 & 22.7\% & 0.702 \\
rotation\_large\_200 & 40 & 3 & 120 & 32 & 26.7\% & 0.789 \\
rotation\_50farms\_6foods & 50 & 3 & 150 & 34 & 22.7\% & 0.731 \\
rotation\_75farms\_6foods & 75 & 3 & 225 & 54 & 24.0\% & 0.717 \\
rotation\_100farms\_6foods & 100 & 3 & 300 & 79 & 26.3\% & 0.764 \\
rotation\_25farms\_27foods & 25 & 3 & 75 & 16 & 21.3\% & 0.768 \\
rotation\_50farms\_27foods & 50 & 3 & 150 & 32 & 21.3\% & 0.684 \\
rotation\_100farms\_27foods & 100 & 3 & 300 & 74 & 24.7\% & 0.784 \\
rotation\_200farms\_27foods & 200 & 3 & 600 & 157 & 26.2\% & 0.834 \\
\midrule
