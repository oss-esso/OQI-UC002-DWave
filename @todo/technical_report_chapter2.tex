...
The binary indicators must be consistent with area allocations. If $A_{f,c} > 0$, then $Y_{f,c} = 1$:

\begin{equation}
A_{f,c} \leq L_f \cdot Y_{f,c} \quad \forall f \in \mathcal{F}, \forall c \in \mathcal{C}
\label{eq:linking}
\end{equation}

This yields another 675 constraints, ensuring that $Y_{f,c} = 0 \implies A_{f,c} = 0$.

\subsubsection{Food Group Constraints}

For each food group $g \in \mathcal{G}$, we enforce diversity and nutritional requirements:

\begin{equation}
\sum_{f \in \mathcal{F}} \sum_{c \in \mathcal{C}: G(c) = g} A_{f,c} \geq \alpha_g \cdot \sum_{f \in \mathcal{F}} L_f
\label{eq:food_group}
\end{equation}

where $\alpha_g$ is the minimum fraction of total land that must be allocated to food group $g$.

The number of food group constraints depends on $|\mathcal{G}|$ (typically 5-8 major groups: grains, vegetables, fruits, proteins, dairy, etc.).
...
& \sum_{f,c: G(c)=g} A_{f,c} \geq \alpha_g \cdot 100 & \forall g \in \mathcal{G} \\
...
Similar to the continuous case, but using patch areas:

\begin{equation}
\sum_{p \in \mathcal{P}} \sum_{c: G(c)=g} s_p \cdot Y_{p,c} \geq \alpha_g \cdot \sum_{p} s_p
...
& \sum_{p,c: G(c)=g} s_p \cdot Y_{p,c} \geq \alpha_g \cdot 100 & \forall g \in \mathcal{G} \\
...
\begin{itemize}
    ...
    \item \textbf{Constraints}: $\approx$ 1375 (25 land + 675 min planting + 675 linking + food groups)
...
& \sum_{f,c: G(c)=g} A_{f,c} \geq \alpha_g \cdot 100 - \epsilon \quad \forall g
\end{aligned}
\end{equation}
