...
def test_cqm_creation():
    ...
    farm_data = generate_farm_data(3, 100.0)
    cqm_farm, A, Y, metadata = create_cqm_farm(
        list(farm_data['land_data'].keys()),
        foods, food_groups, config
    )
    
    # Expected: 3 farms * 27 crops * 2 vars = 162 variables
    # Constraints: 3 land + 3*27 min + 3*27 max + 10 food groups = 177
    assert len(cqm_farm.variables) == 162
    assert len(cqm_farm.constraints) == 177  # Updated count
    
    # Patch scenario: binary only
    patch_data = generate_patch_data(3, 100.0)
    cqm_patch, Y_patch, metadata = create_cqm_plots(
        list(patch_data['land_data'].keys()),
        foods, food_groups, config
    )
    
    # Expected: 3 patches * 27 crops = 81 binary variables
    # Constraints: 3 one-hot + 0 min + 0 max + 10 food groups = 13
    assert len(cqm_patch.variables) == 81
    assert len(cqm_patch.constraints) == 13
\end{lstlisting}
...
\begin{itemize}
    ...
    \item Both include min AND max area/plots constraints where applicable
    \item Both include min AND max food group constraints (count-based)
    \item Food-group constraints operate on selection counts ($Y$ variables)
\end{itemize}
