...
\subsection{Heterogeneous Solver Specialization}

The core principle of Alternative 2 is \textit{problem-solver matching}:

\begin{itemize}
    \item \textbf{Continuous Problems}: Classical solvers excel at mixed-integer nonlinear programming with continuous variables and complex constraints
    \item \textbf{Binary Problems}: Quantum annealers are specifically designed for binary optimization via energy minimization
\end{itemize}

Rather than applying a universal hybrid approach, Alternative 2 recognizes problem structure and routes to the most appropriate solver.
...
\subsection{Farm Scenario: Gurobi MINLP}

For farm scenarios with continuous land allocation variables:

\subsubsection{Problem Characteristics}

\begin{itemize}
    \item \textbf{Variables}: 1350 (675 continuous $A_{f,c}$ + 675 binary $Y_{f,c}$)
    \item \textbf{Constraints}: $\sim$1375 (land, minimum planting, linking, food groups)
    \item \textbf{Objective}: Linear weighted sum
    \item \textbf{Formulation}: Mixed-integer nonlinear program (MINLP)
\end{itemize}

\subsubsection{Gurobi Configuration}

Gurobi is configured for MINLP solving:

\begin{lstlisting}[caption={Gurobi Solver Setup},label={lst:gurobi_setup}]
import pulp as pl

# Create problem
prob = pl.LpProblem("Farm_Allocation", pl.LpMaximize)
...
Gurobi employs advanced MINLP algorithms:

\begin{itemize}
    \item \textbf{Branch-and-Bound}: Systematic enumeration with intelligent pruning
    \item \textbf{Cutting Planes}: Dynamic constraint generation to tighten LP relaxation
    \item \textbf{Presolve}: Problem size reduction via redundancy elimination
    \item \textbf{Heuristics}: Fast feasibility detection and solution improvement
\end{itemize}

Typical solve times for the 25-farm, 27-crop instance: $\sim$0.1-1.0 seconds.
