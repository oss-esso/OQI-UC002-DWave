"""
Professional solver runner script with CUSTOM HYBRID WORKFLOW (dwave-hybrid framework).
...
4. Solves with DWave using CUSTOM HYBRID WORKFLOW (manual hybrid algorithm)
   - Uses dwave-hybrid framework to build bespoke workflow
   - Inspired by Kerberos sampler architecture
   - Decomposes problem, runs classical + QPU samplers, iterates to convergence
...
# Import dwave-hybrid components for custom workflow
try:
    import hybrid
    from hybrid import (
        Loop, Race, Parallel, ArgMin, State,
    ...
    print("Warning: dwave-hybrid not available. Install with: pip install dwave-hybrid")
...
def create_cqm_farm(farms, foods, food_groups, config):
    ...
    if food_group_constraints:
        ...
        for food in foods:
            ...
            # Also: U[c] <= sum(Y[f,c]) ensures U[c]=0 if no Y[f,c]=1
            cqm.add_constraint(
...
def solve_with_pulp_plots(farms, foods, food_groups, config):
    ...
    ## Constraint 4: U-X linking for unique food tracking
    for c in foods:
        for f in farms:
            model += X_pulp[(f, c)] <= U_pulp[c], f"U_Link_{f}_{c}"
        model += U_pulp[c] <= pl.lpSum([X_pulp[(f, c)] for f in farms]), f"U_Bound_{c}"
    
    ## Constraint 5: Food group constraints - count UNIQUE foods
    if food_group_constraints:
        for g, constraints in food_group_constraints.items():
            foods_in_group = food_groups.get(g, [])
            if foods_in_group:
                ...
                if 'min_foods' in constraints:
                    model += pl.lpSum([U_pulp[c] for c in foods_in_group]) >= constraints['min_foods'], f"MinFoodGroup_Unique_{group_label}"
                if 'max_foods' in constraints:
                    model += pl.lpSum([U_pulp[c] for c in foods_in_group]) <= constraints['max_foods'], f"MaxFoodGroup_Unique_{group_label}"
...
def solve_with_custom_hybrid_workflow(cqm, token, **kwargs):
    """
    Solve a CQM using a custom hybrid workflow built with dwave-hybrid framework.
    
    This function creates a bespoke hybrid algorithm inspired by the Kerberos sampler:
    - Decomposes the problem into smaller subproblems
    - Runs classical samplers (tabu, simulated annealing) in parallel with QPU sampling
    - Iterates until convergence or maximum iterations reached
    
    The workflow architecture:
    1. QPU Branch: EnergyImpactDecomposer → QPUSubproblemSampler → SplatComposer
    2. Classical Branches: InterruptableTabuSampler, SimulatedAnnealingSampler
    3. Racing: Classical and quantum branches race, terminated by first completion
    4. Selection: ArgMin selects best sample from racing branches
    5. Iteration: Loop repeats until convergence or max_iter
    
    Args:
        cqm: ConstrainedQuadraticModel to solve
        token: D-Wave API token (string, default: None uses environment variable)
        **kwargs: Additional parameters:
            - subproblem_size (int): Number of variables in QPU subproblem (default: 40)
            - tabu_timeout (int): Tabu search timeout in milliseconds (default: 200)
            - sa_timeout (int): Simulated annealing timeout in milliseconds (default: 200)
            - max_iter (int): Maximum iterations (default: 15)
            - convergence (int): Iterations without improvement before stopping (default: 3)
            - num_qpu_reads (int): Number of QPU reads per iteration (default: 100)
    
    Returns:
        dict: Solution containing:
            - status (str): 'Optimal', 'Converged', or 'Max Iterations'
            - objective_value (float): Best CQM objective value found
            - solve_time (float): Total solve time in seconds
            - qpu_access_time (float): Total QPU access time in seconds
            - iterations (int): Number of iterations completed
            - solver_name (str): 'dwave_custom_hybrid'
            - solution (dict): Best variable assignments
    
    Raises:
        ImportError: If dwave-hybrid is not installed
        Exception: If workflow execution fails
    """
    if not HYBRID_AVAILABLE:
        raise ImportError(
            "dwave-hybrid is required for custom hybrid workflows. "
            "Install with: pip install dwave-hybrid"
        )
    ...
    if use_simulated_annealing:
        print("SOLVING WITH CUSTOM HYBRID WORKFLOW (Simulated Annealing - No QPU)")
        print("Note: Using neal.SimulatedAnnealingSampler for testing without D-Wave token")
    else:
        print("SOLVING WITH CUSTOM HYBRID WORKFLOW (dwave-hybrid framework)")
    print("=" * 80)
    
    # Extract parameters
    subproblem_size = kwargs.get('subproblem_size', 40)
    tabu_timeout = kwargs.get('tabu_timeout', 200)
    sa_timeout = kwargs.get('sa_timeout', 200)
    max_iter = kwargs.get('max_iter', 15)
    convergence = kwargs.get('convergence', 3)
    num_qpu_reads = kwargs.get('num_qpu_reads', 100)
    
    print(f"  Workflow Configuration:")
    print(f"    Subproblem Size: {subproblem_size} variables")
    print(f"    Tabu Timeout: {tabu_timeout} ms")
    print(f"    SA Timeout: {sa_timeout} ms")
    print(f"    Max Iterations: {max_iter}")
    print(f"    Convergence Threshold: {convergence} iterations")
    print(f"    QPU Reads per Iteration: {num_qpu_reads}")
    
    # Convert CQM to BQM for hybrid framework
    ...
    # Build custom hybrid workflow
    print("\n  Building Custom Hybrid Workflow...")
    
    if use_simulated_annealing:
        # Simulated Annealing only (no QPU) - for testing
        ...
        print("    Using SimulatedAnnealingSampler (neal) for testing")
        
        # Use only classical samplers without QPU
        tabu_branch = InterruptableTabuSampler(timeout=tabu_timeout)
        sa_branch = SimulatedAnnealingProblemSampler(num_reads=num_qpu_reads, num_sweeps=1000)
        
        # Racing Branches: Run only classical samplers
        racing_branches = (
            Race(
                tabu_branch,
                sa_branch
        ...
        print("    ✅ Workflow structure (Testing Mode):")
        print("       Racing Branches:")
        print("         - Tabu Search (interruptible)")
        print("         - Simulated Annealing (neal)")
        print("       → ArgMin (select best)")
        print("       → Loop (iterate until convergence)")
    else:
        # Full hybrid workflow with QPU
        # QPU Branch: Decompose → Sample on QPU → Compose
        qpu_branch = (
            EnergyImpactDecomposer(size=subproblem_size, rolling=True, rolling_history=0.85) |
            QPUSubproblemAutoEmbeddingSampler(num_reads=num_qpu_reads, qpu_sampler=None, token=token) |
            SplatComposer()
        )
        
        # Classical Branches: Interruptible samplers
        tabu_branch = InterruptableTabuSampler(timeout=tabu_timeout)
        sa_branch = SimulatedAnnealingProblemSampler(num_reads=1, num_sweeps=1000)
        
        # Racing Branches: Run classical and QPU in parallel, terminate when QPU completes
        racing_branches = (
            Race(
                tabu_branch,
                sa_branch,
        ...
        print("       Racing Branches:")
        print("         - Tabu Search (interruptible)")
        print("         - Simulated Annealing (interruptible)")
        print("         - QPU (EnergyImpactDecomposer → QPU → SplatComposer)")
        print("       → ArgMin (select best)")
        print("       → Loop (iterate until convergence)")
    
    # Loop until convergence or max iterations
    workflow = Loop(racing_branches, max_iter=max_iter, convergence=convergence)
    ...
    # Run workflow
    print("\n  Executing Custom Hybrid Workflow...")
    workflow_start = time.time()
    
    try:
        result_state = workflow.run(initial_state).result()
        solve_time = time.time() - workflow_start
        
        # Extract results
        best_sample = result_state.samples.first
        best_energy = best_sample.energy
        
        # Invert BQM solution back to CQM space
        cqm_sample = invert(best_sample.sample)
        
        # Calculate objective value (negative of energy for maximization)
        objective_value = -best_energy
        
        # Extract QPU timing (accumulated across iterations)
        if use_simulated_annealing:
            qpu_access_time = 0.0  # No QPU used in simulated annealing mode
        else:
            # Note: Extracting QPU time from hybrid workflow is complex
            # We estimate based on num_reads and typical QPU timing
            qpu_access_time = num_qpu_reads * max_iter * 0.0001  # Rough estimate
        
        # Determine status
        if hasattr(workflow, 'iterations_') and workflow.iterations_ < max_iter:
            status = "Converged"
            iterations = workflow.iterations_
        else:
        ...
        print(f"     Estimated QPU Time: {qpu_access_time:.4f}s")
