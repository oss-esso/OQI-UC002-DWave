...
# =============================================================================
# SCIENTIFICALLY OPTIMIZED COLOR PALETTES
# =============================================================================

# PRIMARY QUALITATIVE (10 colors - optimal for ≤10 categories)
# Based on Okabe & Ito colorblind-friendly palette, extended and optimized
QUALITATIVE_PRIMARY: list[str] = [
    "#0072B2",  # Blue (high luminance contrast)
    "#D55E00",  # Vermilion (warm, distinct)
    "#009E73",  # Bluish green
    "#F0E442",  # Yellow (adjusted for better visibility)
    "#56B4E9",  # Sky blue
    "#CC79A7",  # Reddish purple
    "#E69F00",  # Orange
    "#000000",  # Black (for critical series)
    "#999999",  # Neutral gray
    "#924900",  # Brown (extended palette)
]

# EXTENDED QUALITATIVE (20 colors - for complex multi-series plots)
# Optimized in CIELAB space with minimum perceptual distance > 20 ΔE
QUALITATIVE_EXTENDED: list[str] = [
    # Core 10 (same as above for consistency)
    "#0072B2", "#D55E00", "#009E73", "#F0E442", "#56B4E9",
    "#CC79A7", "#E69F00", "#000000", "#999999", "#924900",
    # Extended 10 (carefully spaced in perceptual space)
    "#117733",  # Forest green
    "#332288",  # Indigo
    "#AA4499",  # Purple
    "#44AA99",  # Teal
...
# SEQUENTIAL PALETTES (ORDERED DATA - HEATMAPS, GRADIENTS)
# VIRIDIS-BASED (Perceptually uniform, colorblind-safe)
SEQUENTIAL_VIRIDIS: list[str] = [
    "#440154", "#482878", "#3E4A89", "#31688E", "#26828E",
    "#1F9E89", "#35B779", "#6DCD59", "#B4DE2C", "#FDE725"
]

# PLASMA (High dynamic range, good for highlights)
SEQUENTIAL_PLASMA: list[str] = [
    "#0D0887", "#47039F", "#7301A8", "#9C179E", "#BD3786",
    "#D8576B", "#ED7953", "#FA9E3B", "#FDC926", "#F0F921"
]

# DIVERGING PALETTES (CRITICAL MIDPOINT DATA)
# RD-BU (Red-Blue, balanced midpoint)
DIVERGING_RDBU: list[str] = [
    "#67001F", "#B2182B", "#D6604D", "#F4A582", "#FDDBC7",
    "#F7F7F7",  # Neutral white midpoint
...
# SOLVER HIERARCHY (Based on algorithm type)
SOLVER_COLORS: dict[str, str] = {
    # QPU Methods (Blues)
    "qpu": "#1B9AAA",           # Primary QPU
    "qpu_dark": "#0A6C7D",      # Dark QPU
    "qpu_light": "#7ECFD4",     # Light QPU
    "dwave_hybrid": "#2E86AB",  # Hybrid solver
    
    # Classical Optimizers (Greens)
    "gurobi": "#2CA02C",        # Gurobi
    "pulp": "#1A7B1A",          # PuLP
    "cplex": "#3CB371",         # CPLEX
    
    # Heuristic Methods (Oranges)
    "heuristic": "#FF7F0E",     # General heuristic
    "simulated_annealing": "#FF9A4D",
    "genetic_algorithm": "#FFB47D",
    
    # Semantic Colors (Consistent across plots)
    "optimal": "#000000",       # Optimal/ground truth
    "baseline": "#7F7F7F",      # Baseline method
    "violation": "#E41A1C",     # Constraint violation
    "feasible": "#4DAF4A",      # Feasible solution
    "timeout": "#984EA3",       # Timeout/failure
    "improvement": "#FFD700",   # Improvement highlight
}

# METHOD FAMILY COLORS (Group related methods)
METHOD_COLORS: dict[str, str] = {
    # Quantum/Quantum-inspired
    "QPU": "#1B9AAA",                     # Generic QPU
    "D-Wave Hybrid": "#2E86AB",           # D-Wave hybrid
    "Quantum Annealing": "#117799",
    
    # Decomposition Methods (Purples)
    "PlotBased": "#7570B3",               # Plot-based decomposition
    "PlotBased_QPU": "#8A7CC7",
    "cqm_first_PlotBased": "#9F8CE6",
    
    # Multilevel Methods (Greens)
    "Multilevel": "#66A61E",              # Generic multilevel
    "Multilevel(5)_QPU": "#7EBF3C",
    "Multilevel(10)_QPU": "#96D85A",
    
    # Graph Partitioning (Oranges)
    "Louvain": "#E6AB02",                 # Louvain method
    "Louvain_QPU": "#F2C14E",
    "Spectral": "#A6761D",                # Spectral methods
    "Spectral(10)_QPU": "#BF924A",
    
    # Grid Methods (Teals)
    "HybridGrid": "#4DAF4A",              # Grid-based
    "HybridGrid(5,9)_QPU": "#66C166",
    "HybridGrid(10,9)_QPU": "#80D380",
    
    # Classical Solvers (Blues)
    "Gurobi": "#1F77B4",                  # Gurobi optimal
    "PuLP": "#2CA02C",                    # PuLP/open-source
    "CPLEX": "#386CB0",
    
    # Coordination Methods (Reds)
    "coordinated": "#E31A1C",             # Coordinated methods
    "centralized": "#FB9A99",
    
    # Direct Methods (Browns)
    "direct_qpu": "#8C564B",
    "exact": "#BCBD22",
}

# PROBLEM FORMULATION COLORS
FORMULATION_COLORS: dict[str, str] = {
    "6-Family": "#3182BD",      # Cool blue
    "6family": "#3182BD",
    "27-Food": "#E6550D",       # Warm orange
    "27food": "#E6550D",
    "Basic": "#31A354",         # Green
    "Extended": "#756BB1",      # Purple
}

# FOOD GROUP COLORS (Nutritionally meaningful)
FOOD_GROUP_COLORS: dict[str, str] = {
    "Vegetables": "#33A02C",    # Fresh green
    "Grains": "#FDBF6F",        # Wheat/gold
    "Legumes": "#B2DF8A",       # Light green
    "Fruits": "#FF7F00",        # Citrus orange
    "Meats": "#E31A1C",         # Red meat
    "Dairy": "#6A3D9A",         # Purple (calcium)
    "Fats": "#FFFF99",          # Yellow (oils)
    "Sugars": "#FB9A99",        # Pink
}
...
MARKERS: dict[str, str] = {
    "6-Family": "o",      # Circle (balanced)
    "27-Food": "s",       # Square (structured)
    "QPU": "D",           # Diamond (premium)
    "Gurobi": "^",        # Triangle up (optimal)
    "Heuristic": "v",     # Triangle down (approximate)
    "Hybrid": "p",        # Pentagon (mixed)
    "Baseline": "X",      # X (reference)
}

# LINESTYLES (For method differentiation)
LINESTYLES: dict[str, str] = {
    "optimal": "-",       # Solid (optimal/reference)
    "quantum": "--",      # Dashed (quantum methods)
    "heuristic": "-.",    # Dash-dot (heuristics)
    "baseline": ":",      # Dotted (baselines)
}
