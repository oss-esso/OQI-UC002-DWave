# BQM Formulation Analysis

## 1. Introduction

This document provides a script to analyze and compare the two different BQM formulations that have been used in this project. The discrepancy between these formulations is the root cause of the constraint violation issues observed with the `comprehensive_benchmark.py` script when using a BQM-based solver.

The two formulations are:
-   **"BQUBO" Formulation:** Used in `benchmark_scalability_BQUBO.py`. This is a simpler model based on "binary plantations", where a "farm" is treated as a capacity pool for 1-acre plantations.
-   **"PATCH" Formulation:** Used in `comprehensive_benchmark.py`. This is a more complex and realistic "plot assignment" model where each discrete patch of land can only be assigned to a single crop.

The script below will perform the following actions for a given problem size:
1.  Generate the BQM for both formulations.
2.  Compare their structural complexity (number of variables, quadratic terms, and density).
3.  Solve both BQMs using classical solvers (Gurobi and Simulated Annealing) to avoid using D-Wave credits.
4.  Generate a detailed Markdown report comparing the formulations with a toy example, complexity metrics, and solver results.

## 2. Analysis Script

The following Python script, `analyze_bqm_formulations.py`, contains the complete logic for the comparison. It is designed to be self-contained by borrowing the necessary functions from the project's other scripts.
...
def create_food_config_patch(land_data: Dict[str, float]) -> Tuple[Dict, Dict, Dict]:
    ...
    config = {
        'parameters': {
            ...
            'weights': {
                'nutritional_value': 0.25, 'nutrient_density': 0.2,
                'environmental_impact': 0.25, 'affordability': 0.15,
                'sustainability': 0.15
            },
...
1.  Save the code block above into a file named `analyze_bqm_formulations.py` in your project root directory (`C:\Users\Edoardo\Documents\EPFL\OQI-UC002-DWave`).
2.  Open your terminal or command prompt and navigate to the project directory.
3.  Run the script and redirect the output to a Markdown file. You can specify the problem size (number of farms/patches) using the `--n_units` argument.

    **Example Commands:**

    ```bash
    # For a problem size of 10 units (a smaller, quicker test)
    python analyze_bqm_formulations.py --n_units 10 > analysis_report_10.md

    # For a problem size of 50 units (to match the failing case)
    python analyze_bqm_formulations.py --n_units 50 > analysis_report_50.md
    ```

## 4. Analysis Report

*(This section will be populated with the detailed analysis after you run the command above. The generated `.md` file will contain the full report.)*
