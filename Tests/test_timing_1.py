...
def read_in_args(args):
    ...
    parser.add_argument("-g", "--graph", default='karate', choices=['karate', 'rand-reg', 'ER', 'SF'], help='Graph to partition (default: %(default)s)')
    parser.add_argument("-n", "--nodes", help="Set graph size for graph. (default: %(default)s)", default=20, type=int)
...
def run_cqm_and_collect_solutions(cqm, sampler):
    ...
    print(sampleset.info.get('qpu_access_time', 'No timing info available'))
