...
across problem sizes from 5 to 10000 farms (with runtime caps for the heaviest
Variant~B cases). We compare three solving paradigms:
...
Variant~B assigns one of 27 crops to each farm across $T=3$ time periods.
Let $Y_{f,c,t}\in\{0,1\}$ indicate farm $f$ grows crop $c$ in period $t$.
...
\label{eq:obj-B}
\end{equation}
where the linear terms encode crop benefit scores, and the
quadratic terms encode:
\begin{itemize}
  \item \textbf{Temporal rotation synergies:} $R_{c,c'}$ between consecutive periods
    on the~same~farm,
  \item \textbf{One-hot penalties:} enforcing exactly one crop per farm per period.
\end{itemize}

The decision space has $n = 27FT = 81F$ binary variables.
...
\begin{description}
  \item[Clique (farm-by-farm):] One sub-problem per farm, each with $27T = 81$
  ...
  \item[SpatialTemporal($g$):] Cluster $g$ farms $\times$ one period per partition.
    Partition count: $\lceil F/g\rceil \cdot T$; max sub-problem: $27g$.
\end{description}
...
\begin{subfigure}[b]{0.48\textwidth}
  ...
  \caption{Partition count}
\end{subfigure}
\caption{Variant B decomposition scaling from 5 to 1000 farms (27 crops $\times$ 3 periods).}
\label{fig:decomp-B}
\end{figure}
...
with decomposition itself still fast; the expensive part is solver time on larger
QUBOs. The key difference is in sub-problem \emph{size}: Clique keeps each
sub-problem at a fixed 81 variables, while SpatialTemporal(5) has 135.

%% ====================================================================
\section{Solver Comparison: Whole-Problem vs.\ Decomposed}
\label{sec:solvers}

We compare three solving approaches across farm sizes
$F \in \{5, 10, 25, 50, 100, 200, 500, 1000, 2000, 5000, 10000\}$.
PT-ICM runs are reported up to 200 farms (runtime cap), and Variant~B is skipped
beyond 2000 farms (runtime cap).
...
\caption{Variant B (27-crop rotation) solver wall-clock time comparison across all
  decomposition methods.  Gurobi\_full (red) is the monolithic baseline and is now
  measured up to 2\,000 farms; Gurobi\_decomposed Clique (blue) and SpatialTemporal-5
  (green) are the decomposed alternatives.  Farm sizes above 100 that were previously
  skipped have been added; points that hit the 200\,s timeout are plotted at the timeout
  wall-clock value.}
...
\caption{Variant B solution quality comparison across all decomposition methods.
  Higher objective values indicate better solutions.}
\label{fig:solver-qual-B}
\end{figure}

\subsection{Study 2.B Three-Group Comparison (Track A: 27-crop)}
\label{sec:study2b-track-a}

The following figures extend the Variant~B benchmarks by adding a third group:
the D-Wave QPU hierarchical pipeline from Study~2.B\@.  This enables a direct
wall-clock time and solution-quality comparison across (i)~monolithic Gurobi,
(ii)~the QPU hierarchical solver, and (iii)~Gurobi with farm-level decomposition.

\textbf{Formulation note.} Solver-comparison and QPU runs use the same 27-crop
rotation formulation (canonical Variant~B with \texttt{rotation\_gamma}$= 0.2$,
\texttt{spatial\_gamma}$= 0.1$).  However, QPU \texttt{objective\_miqp} values
are QUBO energies that include one-hot penalty contributions, whereas Gurobi
reports the pure MIQP benefit; these scales differ by a factor of $\approx$5
and are not directly comparable for solution quality.  The wall-clock time
comparison is formulation-independent and fully valid.

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{fig_study2b_time_track_a.pdf}
\caption{Study 2.B Track A — wall-clock time scaling for all three solver groups
  (27-crop Variant~B).  Gurobi full (red solid) is the monolithic baseline;
  QPU hierarchical (orange, diamonds) shows D-Wave wall-clock time including
  embedding and sampling overhead;
  Gurobi decomposed Clique (blue dashed) and SpatialTemporal-5 (green dashed) are
  the farm-level decomposition baselines.}
\label{fig:study2b-time-a}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.97\textwidth]{fig_study2b_quality_track_a.pdf}
\caption{Study 2.B Track A — solution quality comparison (27-crop Variant~B).
  \emph{Left:} absolute MIQP objective values for Gurobi full and decomposed
  (same scenario random grid, directly comparable).
  \emph{Right:} quality ratio relative to Gurobi full (left panel) and QPU
  benefit divided by Gurobi MIQP objective from matched scenario timeout runs
  ($\dagger$\,different objective scales; ratio not a direct quality comparison).}
\label{fig:study2b-qual-a}
\end{figure}

\subsection{Study 2.B Track B: 6-food + 27-crop Combined}
\label{sec:study2b-track-b}

Track~B extends the comparison to both the 6-food (food-group aggregated)
and 27-crop formulations, using the full set of available QPU runs and
the Gurobi timeout verification data for the 6-food scenarios.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig_study2b_time_track_b.pdf}
\caption{Study 2.B Track B — wall-clock time scaling for both 6-food and 27-crop
  formulations.  Gurobi full 27-crop (red solid) and 6-food (red dotted) from
  two different data sources; QPU 27-crop (orange solid, diamonds) and
  QPU 6-food (orange dotted, diamonds); Gurobi decomposed Clique and
  SpatialTemporal-5 for 27-crop (blue/green dashed).}
\label{fig:study2b-time-b}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig_study2b_quality_track_b.pdf}
\caption{Study 2.B Track B — solution quality comparison for both formulations.
  \emph{Left:} absolute Gurobi full objectives (27-crop from solver-comparison,
  6-food from 200\,s timeout verification runs) showing linear scaling with farm
  count.  \emph{Right:} QPU benefit divided by matched Gurobi MIQP objective
  for each formulation ($\dagger$\,different normalizations between formulations).}
\label{fig:study2b-qual-b}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Variant~B:}  For each farm--period pair $(f, t)$, check
    $\sum_{c} Y_{f,c,t} = 1$ (one-hot).
    A violation means a slot has zero or more than one crop assigned.
\end{itemize}
...
\begin{table}[H]
\centering
\caption{PT-ICM constraint feasibility across problem sizes (farm sizes 5--200).
...
A & PlotBased         & 200 & 200 & 0 & 0.0 & 0.279 & 0.279 \\
A & Multilevel(5)     &   5 &   5 & 0 & 0.0 & 0.176 & 0.176 \\
A & Multilevel(5)     &  50 &  50 & 0 & 0.0 & 0.261 & 0.261 \\
A & Multilevel(5)     & 200 & 200 & 0 & 0.0 & 0.244 & 0.244 \\
A & HybridGrid(5,9)   &  10 &  10 & 0 & 0.0 & 0.677 & 0.677 \\
A & HybridGrid(5,9)   &  50 &  50 & 0 & 0.0 & 0.723 & 0.723 \\
A & HybridGrid(5,9)   & 200 & 200 & 0 & 0.0 & 0.742 & 0.742 \\
\midrule
B & Clique            &   5 &  15 & 0 & 0.0 & 0.790 & 0.790 \\
B & Clique            &  10 &  30 & 0 & 0.0 & 0.724 & 0.724 \\
B & Clique            &  50 & 150 & 0 & 0.0 & 0.711 & 0.711 \\
B & Clique            & 200 & 600 & 0 & 0.0 & 0.754 & 0.754 \\
B & SpatialTemporal(5) &  5 &  15 & 0 & 0.0 & 0.754 & 0.754 \\
B & SpatialTemporal(5) & 10 &  30 & 0 & 0.0 & 0.708 & 0.708 \\
B & SpatialTemporal(5) & 50 & 150 & 0 & 0.0 & 0.721 & 0.721 \\
B & SpatialTemporal(5) &200 & 600 & 0 & 0.0 & 0.728 & 0.728 \\
\bottomrule
\end{tabular}
\end{table}
...
(5--200 farms, all decomposition strategies, both variants).  This confirms that
the one-hot penalty terms in the BQM/QUBO formulation are strong enough
($\lambda = 3.0$ in the PT-ICM sub-BQMs for both variants) to guide the
...
\midrule
A & PlotBased        &  10 & 0.266 & 0.255 & 0.956 \\
A & PlotBased        &  50 & 0.346 & 0.266 & 0.769 \\
A & Multilevel(5)    &  10 & 0.266 & 0.211 & 0.793 \\
A & Multilevel(5)    &  50 & 0.346 & 0.261 & 0.755 \\
A & HybridGrid(5,9)  &  10 & 0.266 & 0.677 & 2.545$^\dagger$ \\
A & HybridGrid(5,9)  &  50 & 0.346 & 0.723 & 2.091$^\dagger$ \\
\midrule
B & Clique           &  10 & $-91.3$ & 0.724 & $-0.0079$ \\
B & Clique           &  50 & $-451.3$ & 0.711 & $-0.0016$ \\
B & SpatialTemporal(5) &10 & $-91.3$ & 0.708 & $-0.0078$ \\
B & SpatialTemporal(5) &50 & $-451.3$ & 0.721 & $-0.0016$ \\
\bottomrule
\end{tabular}

\smallskip
$^\dagger$HybridGrid decomposed-Gurobi is also above the monolithic baseline,
so the PT-ICM ratio appears $>1$ against monolithic Gurobi; this reflects
decomposition relaxation, not true global optimality.
\end{table}

\paragraph{Interpretation.}
For Variant~A, PT-ICM recovers $\sim$75--96\% of the monolithic Gurobi objective
for PlotBased/Multilevel at 10--50 farms, which is reasonable given 500--800 sweeps
...
negative-energy states (e.g.\ $-451.3$ at 50~farms) while PT-ICM settles on
shallow positive-energy configurations ($\sim+0.7$).  This is because
...
couple multiple farms (135 variables per partition), creating denser
...
\begin{enumerate}
  \item \textbf{Decomposition overhead} is negligible (milliseconds for the
    linear-time partitioners even at large sizes) except for Spectral and Louvain,
    which incur $O(n^2)$ and
  ...
  \item \textbf{Decomposed Gurobi} achieves near-optimal solutions much faster than
    ...
    couplings.
  \item \textbf{PT-ICM} provides competitive quality on small-to-medium sub-problems
    (here benchmarked up to 200 farms due to runtime caps) but scales poorly on
    larger sub-problems due to the
    $O(n^2)$ per-sweep cost of the Ising energy evaluation.
  \item \textbf{D-Wave QPU (Study~2.B three-group comparison):}
    The wall-clock time of the QPU hierarchical pipeline lies between
    Gurobi full (slowest at large scales) and Gurobi decomposed (fastest).
    For the 27-crop formulation, QPU total wall time scales approximately
    linearly with $n_{\mathrm{farms}}$ up to 200 farms, dominated by
    embedding and sampling overhead rather than problem complexity.
    For the 6-food formulation, QPU wall time similarly outpaces Gurobi full
    (which hits the 200\,s timeout for $\gtrsim 50$ farms) but is slower
    than farm-level Gurobi decomposition.
  \item \textbf{Gurobi full extended baseline:} Full MIQP Gurobi for Variant~B
    is now measured up to 2\,000 farms (27-crop): solve times remain within
    the 200\,s timeout for $\le 200$ farms but hit the timeout at 500 farms and
    above, returning feasible partial solutions.  This confirms that decomposition
    is necessary for large-scale 27-crop rotation problems.
